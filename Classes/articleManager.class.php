...
class articleManager
{
	...
	{
		$req = $this->_db->prepare('select * from consulter inner join article on article.id_article = consulter.id_article WHERE article.id_categorie = ? and publier = 1 and ID_ARTICLE not in (select ID_ARTICLE from deletedArticle) group by consulter.id_article order by sum(vue) DESC LIMIT 0,5');
	...
	{
		$req = $this->_db->query('SELECT * FROM `article` WHERE publier = 1 and ID_ARTICLE not in (select ID_ARTICLE from deletedArticle) order by date_realisation desc limit 0,6');
		while($donnees = $req->fetch(PDO::FETCH_ASSOC))
		{
			$articles[] = new article($donnees);
		}
	...
	{
		$req = $this->_db->prepare('SELECT * FROM article WHERE ID_UTILISATEUR = ? and ID_ARTICLE not in (select ID_ARTICLE from deletedArticle) order by DATE_REALISATION DESC');
	...
	{
		$req = $this->_db->query('SELECT * from article where ID_ARTICLE not in (select ID_ARTICLE from deletedArticle) order by DATE_REALISATION DESC');
		while($donnees = $req->fetch(PDO::FETCH_ASSOC))
		{
			$articles[] = new article($donnees);
		}
	...
	{
		$req = $this->_db->prepare('SELECT * from article WHERE Publier = 1 and ID_ARTICLE not in (select ID_ARTICLE from deletedArticle) order by DATE_REALISATION DESC LIMIT :min, :max');
	...
	public function getListByCategorie($idCategorie,$min,$max)
	{
		...
		$req = $this->_db->prepare('SELECT * from article WHERE ID_CATEGORIE = :idCategorie and Publier = 1 and ID_ARTICLE not in (select ID_ARTICLE from deletedArticle) order by DATE_REALISATION DESC LIMIT :min, :max');
		$req->bindValue(':min',$min,PDO::PARAM_INT);
		$req->bindValue(':max',$max,PDO::PARAM_INT);
	...
	{
		$req = $this->_db->query('SELECT * FROM `article` WHERE publier = 1 and ID_ARTICLE not in (select ID_ARTICLE from deletedArticle) order by date_realisation desc limit 6,10');
		$donnees = $req->fetch(PDO::FETCH_ASSOC);
	...
	{
		$req = $this->_db->prepare('SELECT * FROM `article` WHERE PUBLIER = 1 and ID_CATEGORIE = ? and ID_ARTICLE != ? and ID_ARTICLE not in (select ID_ARTICLE from deletedArticle) order by date_realisation desc limit 0,10');
	...
	{
		$req = $this->_db->query('SELECT count(*) as nombreArticle FROM article WHERE publier = 1 and ID_ARTICLE not in (select ID_ARTICLE from deletedArticle)');
		$donnees = $req->fetch(PDO::FETCH_ASSOC);
	...
	{
		$req = $this->_db->prepare('SELECT count(*) as nombreArticle FROM article WHERE ID_CATEGORIE = ? and Publier = 1 and ID_ARTICLE not in (select ID_ARTICLE from deletedArticle)');
	...
	{
		$query = $this->_db->prepare('INSERT INTO deletedArticle set ID_ARTICLE = ?');						
		$query->execute(array($article->ID_ARTICLE()));
	}
